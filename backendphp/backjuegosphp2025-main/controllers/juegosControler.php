...
class JuegosController {
    ...
    public function getById($id) {
        global $pdo;
        try {
            $stmt = $pdo->prepare("SELECT * FROM juegos WHERE idjuegos = ?");
            $stmt->execute([$id]);
    ...
    public function create($data) {
        ...
        try {
            ...
            // 4. Insertar
            $stmt = $pdo->prepare("INSERT INTO juegos (nombre, descripcion, idestatus) VALUES (?, ?, ?)");
    ...
    public function update($id, $data) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT idjuego FROM generos WHERE idgenero = ?");
            $stmt->execute([$id]);
            if (!$stmt->fetch()) {
                http_response_code(404);
